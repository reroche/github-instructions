...
Here are a few ways to use the `git add` command: (1) Adding files individually, (2) Adding all previously tracked files, (3) Adding all files (both tracked and untracked).

(1) Adding files individually
> ```bash
> $ git add draft.Rmd draft.html
> ```
_NOTE:_ If you include multiple files, they must be separated by a space.

(2) Adding all previously tracked files
> ```bash
> $ git add .
> ```

(3) Adding all files (both tracked and untracked)
> ```bash
> $ git add -A
> ```
